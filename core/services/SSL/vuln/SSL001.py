...
from ..sslutils import ssl_recvmsg
...
class SSL001PluginDef():
	def __init__(self):
		self.classname        = 'SSL001'
		self.version          = '0.1'
		self.triggerPorts     = '443'
		self.shortname        = 'SSL Heartbleed'
		self.name             = 'Tests for SSL Heartbleed'
		self.CVE_score        = 5
...
class SSL001():
	"""Tests for SSL heartbleed vulnerability (cve-2014-0160)
	...
	def __init__(self, target = None, args = None):
		self.args            = args
		self.plugindef       = SSL001PluginDef()
		self.target          = target
	...
	def test(self):
		...
		try:
			...
			soc.sendall(hello)
			while True:
				typ, ver, pay, desc = ssl_recvmsg(soc, self.args['socket']['timeout'])
				if typ == None:
